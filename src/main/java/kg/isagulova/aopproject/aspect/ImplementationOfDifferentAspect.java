package kg.isagulova.aopproject.aspect;
...
public class ImplementationOfDifferentAspect {
    ...
    @Pointcut(value = "execution(* kg.isagulova.aopproject.service.ServiceExample.*(..))")
    private void printLogs() {}
    // If there is no @Around advice, @Before will be called
    // first, otherwise @Around Before Invocation is called
    @Before("execution(* kg.isagulova.aopproject.service.ServiceExample.*(..))")
    ...
    @After("execution(* kg.isagulova.aopproject.service.ServiceExample.*(..))")
    ...
    @AfterReturning(value = "execution(* kg.isagulova.aopproject.service.ServiceExample.*(..))", returning = "account")
    ...
    // occurs
    @AfterThrowing(value = "execution(* kg.isagulova.aopproject.service.ServiceExample.*(..))", throwing = "ex")
